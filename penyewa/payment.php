    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-sm font-black text-slate-800 uppercase italic tracking-tighter">Secure Checkout</h1>
            </div>
            <div class="flex items-center gap-3 bg-amber-50 px-4 py-1.5 rounded-xl border border-amber-100">
                ...
                <p class="text-[10px] font-black text-slate-700 italic" id="timer">23 : 59 : 59</p>
            </div>
        </div>
                ...
                <div class="bg-white rounded-[35px] border border-slate-100 shadow-sm overflow-hidden w-full flex flex-col">
                    <div class="bg-slate-50/50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-[11px] font-black text-slate-700 italic"><?= $deadline ?></p>
                        </div>
                    ...
                    <div id="snap-container" class="w-full flex-grow bg-white relative overflow-hidden">
                        <div id="loading-payment" class="absolute inset-0 flex flex-col items-center justify-center bg-white z-50">
                            <div class="w-8 h-8 border-3 border-primary border-t-transparent rounded-full animate-spin mb-3"></div>
                            <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest text-center">Menyiapkan<br>Metode Pembayaran</p>
                        </div>
                    </div>

                    <div class="px-6 py-3 border-t border-slate-50 bg-slate-50/30 flex justify-between items-center">
                        <p class="text-[8px] text-slate-500 font-black uppercase tracking-widest italic">Griya Asri Kos &copy; 2026</p>
                    ...
                    <div class="px-8 py-8 border-t border-slate-50 bg-slate-50/20 mt-auto">
                        <div class="flex flex-col gap-5">
                            ...
                            <div class="flex justify-between items-center pt-4 border-t border-slate-100">
                                <p class="text-[8px] text-slate-300 font-black uppercase tracking-[0.4em] italic leading-none">AMAN & TERPERCAYA SINCE &copy; 2020</p>
                ...
                <div class="bg-white rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden w-full flex flex-col justify-between">
                    ...
                    <div>
                        <div class="px-6 py-5 border-b border-slate-300 bg-slate-50/50">
                            <h3 class="text-[10px] font-black text-slate-800 italic uppercase tracking-[0.2em] flex items-center gap-2 text-left">
                        ...
                        <div class="p-7 space-y-6">
                            ...
                            <div class="space-y-4 pt-2 text-left">
                                ...
                                <div class="flex justify-between items-center border-b border-dashed border-slate-100 pb-3">
                                    <span class="text-[10px] font-bold text-slate-500 italic">Periode Sewa</span>
                                    <span class="text-[10px] font-black text-slate-800 italic"><?= $start_date ?> - <?= $end_date ?></span>
                                </div>

                                <div class="flex justify-between items-center border-b border-dashed border-slate-100 pb-3">
                                    <span class="text-[10px] font-bold text-slate-500 italic">Durasi Total</span>
                                    <span class="text-[10px] font-black text-slate-800 italic"><?php echo isset($days) ? $days : (isset($diff) ? $diff->days : '1'); ?> Hari</span>
                                </div>

                                <div class="flex justify-between items-center border-b border-dashed border-slate-100 pb-3">
                                    <span class="text-[10px] font-bold text-slate-500 italic">Tipe Kamar</span>
                                    <span class="text-[10px] font-black text-slate-800 italic uppercase tracking-tighter"><?= $room['type_name'] ?></span>
                                </div>

                                <div class="py-2 space-y-3">
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-map-marker-alt text-[10px] text-primary"></i>
                                        <span class="text-[10px] font-bold italic text-slate-500">Surabaya, Jawa Timur</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-wifi text-[10px] text-slate-400"></i>
                                        <span class="text-[10px] font-bold italic text-slate-400">Free WiFi & Listrik</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-shield-check text-[10px] text-emerald-500"></i>
                                        <span class="text-[10px] font-bold italic text-emerald-600">Terverifikasi Griya Asri</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 border-t-4 border-double border-slate-100">
                            ...
                            <div class="flex items-center justify-between bg-slate-900 p-5 rounded-2xl shadow-lg shadow-primary/20 mb-3">
                                <div>
                                    ...
                                    <p class="text-xl font-black text-white tracking-tighter italic leading-none">Rp <?= number_format($total_price) ?></p>
                                </div>
                            ...
                            <a href="javascript:history.back()" class="flex items-center justify-center gap-3 w-full p-5 bg-slate-100 border border-slate-200 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-red-100 hover:text-red-600 transition-all shadow-sm">
                                <i class="fas fa-times-circle text-xs"></i>
                                Batalkan Pesanan
                            </a>
                        </div>

                        <p class="text-[8px] text-slate-300 font-black uppercase tracking-[0.4em] italic text-center pt-2">Griya Asri Kos &copy; 2026</p>
                    </div>
                ...
                if (data.token) {
                    ...
                    window.snap.embed(data.token, {
                        ...
                        onSuccess: function(result) {
                            window.location.href = 'transaction_success.php?order_id=' + result.order_id;
                        },
